...
# combines a number of sim matrices via weighted sum, with weights being set by standard deviation
# returns final shared similarity matrix
def combine_sims_var(sim_matrices, masks, clip=True):
    ...
    for sim, mask in zip(sim_matrices, masks):
        ...
        # if there are no valid entries in a matrix set variance and coverage as 0
        if valid_entries.size == 0:
            variances.append(0)
            coverages.append(0)
        # calculate coverage and variance over valid entries
        else:
